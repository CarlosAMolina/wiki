...
Una vez activado, no puede deshabilitarse, se puede suspender el versionado (cada versión del objeto seguirá ocupando espacio) y más tarde volver a activarlo.

Los objetos en un bucket tienen un atributo ID; con valor nulo de estar el versionado deshabilitado y de estar activado es un valor incremental.

Con el versionado activado, un objeto no se elimina, sino que se añade un marcador de borrado (este marcador es una nueva versión del objeto); de eliminar esta marca vuelve a estar disponible la última versión. Para eliminar un objeto con versionado, especificamos el ID de la versión a eliminar.
